...
class _MockAgent:
    """Returns plausible mock predictions without loading a model.

    Scores are generated by adding Gaussian noise to the ground-truth risk
    score so that the mock ROC curves are realistic (AUC ~ 0.80) and suitable
    for end-to-end pipeline testing.  Scores are still random — just correlated
    with the true label so evaluation metrics make sense.
    """
    ...
    def analyze_all(self, companies_df, sections_dir: str):
        ...
        for _, row in companies_df.iterrows():
            # Use ground-truth risk_score (0-100) if available, else derive from label.
            if "risk_score" in row and not pd.isna(row["risk_score"]):
                true_score = float(row["risk_score"])
            elif "label" in row:
                true_score = 75.0 if int(row["label"]) == 1 else 25.0
            else:
                true_score = 50.0

            # Add noise — approach-specific to give each a slightly different AUC.
            noise_std = {"baseline": 18, "rag": 22, "lora": 20}.get(self.approach, 20)
            score = float(min(95, max(5, true_score + random.gauss(0, noise_std))))
            score = round(score, 1)
